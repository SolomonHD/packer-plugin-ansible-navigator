...
func handleNavigatorEvent(ui packersdk.Ui, e *NavigatorEvent, summary *Summary, verbose bool) {
	switch e.Event {
	...
	case "runner_on_ok":
		if e.Task != "" && e.Host != "" {
			...
			// Include stdout/stderr if verbose is enabled
			if verbose {
				if e.Stdout != "" {
					ui.Message(fmt.Sprintf("    stdout: %s", e.Stdout))
				}
				if e.Stderr != "" {
					ui.Message(fmt.Sprintf("    stderr: %s", e.Stderr))
				}
			}
		}
		summary.TasksTotal++
	...
	case "runner_on_failed":
		if e.Task != "" && e.Host != "" {
			...
			// Always show stdout/stderr for failed tasks
			if e.Stdout != "" {
				ui.Error(fmt.Sprintf("    stdout: %s", e.Stdout))
			}
			if e.Stderr != "" {
				ui.Error(fmt.Sprintf("    stderr: %s", e.Stderr))
			}
	...
	case "runner_on_unreachable":
		if e.Task != "" && e.Host != "" {
			...
			// Always show stdout/stderr for unreachable hosts
			if e.Stdout != "" {
				ui.Error(fmt.Sprintf("    stdout: %s", e.Stdout))
			}
			if e.Stderr != "" {
