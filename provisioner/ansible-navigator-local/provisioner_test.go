...
func TestProvisionerPrepare_DecodesNavigatorConfigAnsibleConfigNewSections(t *testing.T) {
	...
	require.NoError(t, p.Prepare(config))
	require.NotNil(t, p.config.NavigatorConfig)
	require.NotNil(t, p.config.NavigatorConfig.AnsibleConfig)

	ac := p.config.NavigatorConfig.AnsibleConfig
	require.NotNil(t, ac.PrivilegeEscalation)
	require.True(t, ac.PrivilegeEscalation.Become)
	require.Equal(t, "sudo", ac.PrivilegeEscalation.BecomeMethod)
	require.Equal(t, "root", ac.PrivilegeEscalation.BecomeUser)

	require.NotNil(t, ac.PersistentConnection)
	require.Equal(t, 30, ac.PersistentConnection.ConnectTimeout)

	require.NotNil(t, ac.Inventory)
	require.Equal(t, []string{"ini", "yaml"}, ac.Inventory.EnablePlugins)

	require.NotNil(t, ac.ParamikoConnection)
	require.Equal(t, "ssh -W %h:%p jumphost", ac.ParamikoConnection.ProxyCommand)

	require.NotNil(t, ac.Colors)
	require.True(t, ac.Colors.ForceColor)

	require.NotNil(t, ac.Diff)
	require.True(t, ac.Diff.Always)
	require.Equal(t, 3, ac.Diff.Context)

	require.NotNil(t, ac.Galaxy)
	require.Equal(t, []string{"automation_hub"}, ac.Galaxy.ServerList)
	require.True(t, ac.Galaxy.IgnoreCerts)
}
