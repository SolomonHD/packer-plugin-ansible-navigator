...
func TestProvisionerPrepare_ExecutionEnvironment(t *testing.T) {
	var p Provisioner
	config := testConfig(t)
	defer os.Remove(config["command"].(string))
	...
	if p.config.ExecutionEnvironment != "quay.io/ansible/creator-ee:latest" {
		t.Fatalf("execution_environment not set correctly: got %s", p.config.ExecutionEnvironment)
	}
}

func TestExecutionEnvironmentInArgs(t *testing.T) {
	testCases := []struct {
		name                 string
		...
		{
			name:                 "With execution environment",
			executionEnvironment: "quay.io/ansible/creator-ee:latest",
			expectInArgs:         true,
		},
		{
			name:                 "Without execution environment",
			executionEnvironment: "",
			expectInArgs:         false,
		},
		{
			name:                 "With custom execution environment",
			executionEnvironment: "my-registry.io/ansible-ee:v1.0",
			expectInArgs:         true,
		},
	}
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if p.config.ExecutionEnvironment != tc.executionEnvironment {
				t.Fatalf("execution_environment not set correctly: expected %s, got %s",
					tc.executionEnvironment, p.config.ExecutionEnvironment)
			}
		})
	}
}
