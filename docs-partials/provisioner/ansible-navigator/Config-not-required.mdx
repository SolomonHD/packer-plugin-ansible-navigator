...
- `bastion_host` (string) - Bastion (jump host) address for SSH tunneling mode.
  Required when connection_mode is "ssh_tunnel".
  Example: "bastion.example.com"

- `bastion_port` (int) - SSH port on the bastion host. Defaults to 22.

- `bastion_user` (string) - SSH username for authenticating to the bastion host.
  Required when ssh_tunnel_mode is true.

- `bastion_private_key_file` (string) - Path to the SSH private key file for bastion authentication.
  Either this or bastion_password must be provided when ssh_tunnel_mode is true.
  Supports HOME expansion (~ and ~/path).

- `bastion_password` (string) - Password for bastion authentication.
