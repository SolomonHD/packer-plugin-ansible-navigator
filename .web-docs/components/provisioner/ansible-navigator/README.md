...
The `ansible-navigator` Packer provisioner runs Ansible playbooks using [Ansible Navigator](https://ansible.readthedocs.io/projects/navigator/), a text-based user interface (TUI) for running and developing Ansible content. It dynamically creates an Ansible inventory file configured to use SSH, runs an SSH server, executes `ansible-navigator run`, and marshals Ansible plays through the SSH server to the machine being provisioned by Packer.
...
The Ansible provisioner supports automatic installation and management of Ansible collections. This feature ensures that required collections are available before playbook execution, improving reproducibility and simplifying dependency management.

### Configuration Options

The following options control managed collections behavior:

- `collections` ([]string) - List of collections to install. Each entry can be:
  - A collection name: `"community.general"`
  - A collection with version: `"community.general:5.11.0"`
  - A local path: `"myorg.mycollection@/path/to/collection"`
  - A Git repository: `"git+https://github.com/myorg/ansible-collection-myapp.git,main"`
  - A Git repository with specific version: `"git+https://github.com/myorg/ansible-collection-myapp.git,v1.2.0"`

- `collections_cache_dir` (string) - Directory to cache downloaded collections. Defaults to `~/.packer.d/ansible_collections_cache`.

- `collections_offline` (bool) - When true, skip network operations and only use cached collections. Fails if a required collection is not present locally. Default: `false`.

- `collections_force_update` (bool) - When true, always reinstall collections even if cached. Default: `false`.

- `requirements_file` (string) - Path to a `requirements.yml` file for installing collections. **Mutually exclusive with `collections`** - you must use one or the other, not both.
...
**Local Collections with Force Update**
...
**Collections from Git Repository**

```hcl
provisioner "ansible-navigator" {
  collections = [
    "git+https://github.com/myorg/ansible-collection-myapp.git,main",
...
- Collections are installed before playbook execution
- The `ANSIBLE_COLLECTIONS_PATHS` environment variable is automatically set to include the cache directory
- In offline mode, all required collections must be pre-cached
- Collections are validated by checking for `MANIFEST.json` after installation
- The cache directory is created automatically if it doesn't exist
- Collections are cached between Packer runs for faster subsequent builds

## Advanced Examples with Plays and Collections

The following examples demonstrate real-world scenarios combining the `plays` configuration with managed collections.

### Using Collection Plays with Auto-Installation

Execute plays directly from collections with automatic installation:
...
Deploy applications using multiple plays from custom collections:
  ...
  plays = [
    {
      ...
      target = "integration.portainer.setup_environment"
    },
    {
      name = "Configure Docker Swarm"
      target = "integration.portainer.configure_swarm"
    },
    {
      name = "Deploy Application Stack"
      target = "integration.portainer.deploy_stack"
      extra_vars = {
    ...
    },
    {
      ...
      target = "integration.portainer.verify_deployment"
    }
  ]
  
  collections = [
    "integration.portainer@/opt/custom/collections/portainer",
...
provisioner "ansible-navigator" {
  plays = [
    ...
    },
    {
      ...
      target = "acme.security.harden_system"
      extra_vars = {
    ...
    },
    {
      ...
      target = "acme.monitoring.setup_agents"
      extra_vars = {
  ...
  plays = [
    {
      ...
      target = "myorg.infrastructure.deploy"
      extra_vars = {
        environment = "production"
        git_branch = "main"
      }
    },
    {
      ...
      target = "myorg.infrastructure.configure"
    }
  ]
...
### Production Setup with Requirements File

Complex production deployment using requirements file and multiple plays:
...
provisioner "ansible-navigator" {
  ...
  plays = [
    {
      name = "Harden System Security"
      target = "baseline.security.harden_system"
    },
    {
      name = "Setup Monitoring Agents"
      target = "baseline.monitoring.setup_agents"
      extra_vars = {
    ...
    },
    {
      ...
      target = "baseline.logging.configure_rsyslog"
    },
    {
    ...
    {
      name = "Configure SSL"
      target = "app.webapp.configure_ssl"
      extra_vars = {
        ssl_enabled = true
      }
    }
...
Example `requirements.yml` for the above:
...
Deploy and configure containers using collection plays:
  ...
  plays = [
    {
      ...
      target = "community.docker.docker_compose"
      extra_vars = {
...
Combine traditional playbook with managed collections for maximum flexibility:
...
1. **Specify the full path to ansible-navigator:**
...
2. **Add ansible-navigator to PATH:**
...
provisioner "ansible-navigator" {
  playbook_file = "site.yml"
  ansible_navigator_path = ["~/.asdf/shims", "~/.local/bin"]
}
```
...
provisioner "ansible-navigator" {
  playbook_file = "site.yml"
  version_check_timeout = "120s"  # 2 minutes
}
```
...
4. **Skip the version check entirely:**
...
When ansible-navigator is installed via [asdf](https://asdf-vm.com/), additional configuration may be needed because asdf shims may not work correctly in subprocess contexts.

**Recommended Configuration for asdf Users:**
...
provisioner "ansible-navigator" {
  ...
  # Option 1: Use full path to shim
  command = "~/.asdf/shims/ansible-navigator run"
  
  # Option 2: Add shims directory to PATH
  # ansible_navigator_path = ["~/.asdf/shims"]
  
  # Increase timeout if needed for first-time execution environment pulls
...
**Verifying your asdf setup:**

```bash
# Check that ansible-navigator is available
asdf current ansible-navigator

# Test the shim directly
~/.asdf/shims/ansible-navigator --version

# If using asdf and encountering issues, you can also install via pip/pipx
pipx install ansible-navigator
```

### Packer Hangs During Gathering Facts
