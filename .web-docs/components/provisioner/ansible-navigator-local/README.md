...
The local provisioner includes a `version_check_timeout` option that is reserved for future use. Currently, version checking is not performed on the target machine for the local provisioner, but this field maintains consistency with the remote provisioner.
...
provisioner "ansible-navigator-local" {
  ...
  version_check_timeout = "60s"  # Reserved for future use
}
```
...
**Note:** The `version_check_timeout` field is accepted in configuration but has no effect in the current version. Version checking on the target machine will be implemented in a future release.

### asdf Installation on Target Machine

When ansible-navigator is installed on the target machine via [asdf](https://asdf-vm.com/), you may need to ensure the shim directory is accessible:
...
provisioner "ansible-navigator-local" {
  ...
  # Add asdf shims to PATH on the target
...
### Ensuring ansible-navigator is Installed

Since the local provisioner requires ansible-navigator to be installed on the target machine, you typically need to install it before running the provisioner:
...
build {
  ...
  # Install ansible-navigator first
  provisioner "shell" {
    inline = [
      "pip install ansible-navigator",
      # or for asdf:
      # "asdf plugin add ansible-navigator",
      # "asdf install ansible-navigator latest",
      # "asdf global ansible-navigator latest"
    ]
  }
  
  # Then run the local provisioner
  provisioner "ansible-navigator-local" {
    playbook_file = "site.yml"
  }
}
