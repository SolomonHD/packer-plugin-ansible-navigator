...
The on-target provisioner SHALL support a per-play `extra_args` field (list(string)) that is appended to the `ansible-navigator run` remote shell command for that play.
