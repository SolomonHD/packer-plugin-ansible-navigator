...
#### Scenario: pull_policy = "never" prevents Docker image pulls
...
- **AND** the image "my-local-image:latest" exists locally in Docker
...
- **AND** no "Unable to find image locally" errors SHALL occur
- **AND** no Docker registry connection attempts SHALL be made
...
- **AND** the file SHALL be immediately recognized as Version 2 by ansible-navigator 25.12.0+
- **AND** no migration prompts or warnings SHALL appear
