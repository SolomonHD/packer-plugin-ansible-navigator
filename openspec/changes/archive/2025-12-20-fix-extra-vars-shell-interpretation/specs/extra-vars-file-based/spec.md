# extra-vars-file-based Specification Delta

## Purpose

Defines the file-based extra vars passing mechanism to prevent shell interpretation issues when ansible-navigator invokes ansible-playbook inside execution environment containers.
...
This spec documents a file-based extra vars mechanism that is implemented by both the remote and local provisioners.

The existing capabilities specs ([`remote-provisioner-capabilities`](../../specs/remote-provisioner-capabilities/spec.md), [`local-provisioner-capabilities`](../../specs/local-provisioner-capabilities/spec.md)) describe extra vars being passed as "a single JSON object" but do not specify *how* it is passed. This spec clarifies that the JSON SHOULD be written to a temporary file and passed via the `@filepath` syntax.

## Backward Compatibility

This change is **fully backward compatible** from a user perspective:

- No HCL configuration changes required
- No new configuration options added
- Only the internal mechanism for passing extra vars changes
- Users may notice the change in debug logs (file path instead of inline JSON)
