...
* **Connection Mode Enum Replaces Dual Fields**: The confusing dual-field connection configuration has been replaced with a single, explicit `connection_mode` enum:
  * **REMOVED**: `use_proxy` (Trilean), `ssh_tunnel_mode` (bool)
  * **ADDED**: `connection_mode` (string enum: `"proxy"`, `"ssh_tunnel"`, `"direct"`)
  * **Default**: `"proxy"` (equivalent to old `use_proxy = true`)
  * **Migration Required**: Update your Packer HCL configurations:

    ```hcl
    # Before (v3.x)
    provisioner "ansible-navigator" {
      use_proxy = true  # or omit (default)
      # ...
    }
    
    # After (v4.x)
    provisioner "ansible-navigator" {
      connection_mode = "proxy"  # or omit (default)
      # ...
    }
    ```

    ```hcl
    # Before (v3.x) - SSH tunnel mode
    provisioner "ansible-navigator" {
      use_proxy       = false
    ...
    ```hcl
    # Before (v3.x) - Direct mode
    provisioner "ansible-navigator" {
      use_proxy       = false
      ssh_tunnel_mode = false  # or omit
      # ...
    }
    
    # After (v4.x) - Direct mode
    provisioner "ansible-navigator" {
  ...
  * **Rationale**: The previous design used two separate fields to control mutually exclusive connection modes, requiring users to set `use_proxy = false` to enable `ssh_tunnel_mode`, creating a confusing double-negative pattern. The new single enum field explicitly expresses connection intent without backwards compatibility concerns (per project policy in [`openspec/project.md`](openspec/project.md)).
